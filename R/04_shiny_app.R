...
climate_m    <- read_csv("data/processed/climate_monthly.csv", show_col_types = FALSE)
...
custom_css <- HTML("
  ...
  * { font-family: 'Inter', sans-serif !important; }
  ...
  .box {
    ...
    transition: box-shadow 0.3s ease; margin-bottom: 20px;
  }
  .box:hover { box-shadow: 0 4px 20px rgba(0,0,0,0.1); }
  .box-header {
    border-bottom: 1px solid #f0f0f0; padding: 15px 20px;
    border-radius: 12px 12px 0 0; background: #ffffff;
  }
  ...
  .btn-danger {
    ...
    transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(230, 57, 70, 0.3);
  }
  .btn-danger:hover {
    transform: translateY(-2px); box-shadow: 0 6px 20px rgba(230, 57, 70, 0.4);
  }

  .form-control {
    border-radius: 8px; border: 1.5px solid #e0e0e0; padding: 10px 14px;
...
ui <- dashboardPage(
  ...
  dashboardHeader(
    title = span(icon("virus"), span("Malaria Predictor", style = "font-weight: 700;")),
    titleWidth = 260,
    tags$li(class = "dropdown",
      tags$a(href = "https://github.com/calvinokoth9528-cloud/malaria-outbreak-predictor",
             target = "_blank", icon("github"), style = "color: #555; font-size: 18px; padding: 15px;"))
  ),

  dashboardSidebar(
    width = 260,
    tags$div(style = "padding: 20px 15px; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.1);",
      tags$div(style = "font-size: 28px;", icon("mosquito")),
      tags$div(style = "color: #fff; font-size: 12px; margin-top: 5px; opacity: 0.7;", "KEMRI-Inspired Surveillance")
    ),
    sidebarMenu(id = "tabs",
      menuItem("Overview",      tabName = "overview",     icon = icon("chart-line")),
      menuItem("Climate",      tabName = "climate",      icon = icon("cloud-sun-rain")),
      menuItem("Regional",     tabName = "regional",     icon = icon("globe-africa")),
    ...
    tags$div(style = "position: absolute; bottom: 0; width: 100%; padding: 15px; font-size: 11px; color: rgba(255,255,255,0.5); text-align: center; border-top: 1px solid rgba(255,255,255,0.1);",
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "overview",
        ...
        fluidRow(
          box(title = span(icon("chart-line"), "Malaria Incidence Trend (2000-2024)"),
          ...
          box(title = span(icon("info-circle"), "Key Insights"),
              status = "info", solidHeader = FALSE, width = 4,
              tags$div(style = "line-height: 1.8;",
                tags$p(tags$strong("25-Year Journey"), style = "font-size: 15px;"),
                ...
                tags$div(style = "background: #f0f7ff; padding: 10px; border-radius: 8px; border-left: 3px solid #1565c0;",
                  tags$strong("Top Predictor: "), "Last year's incidence is the single strongest predictor of next year's malaria burden.")
              ))
        ),
        fluidRow(
          box(title = span(icon("chart-bar"), "Cases & Deaths Over Time"),
      ...
      tabItem(tabName = "climate",
        fluidRow(
          box(title = span(icon("thermometer-half"), "Temperature vs Malaria"),
              status = "primary", solidHeader = FALSE, width = 6,
              plotlyOutput("plot_temp_malaria", height = "380px")),
          box(title = span(icon("cloud-rain"), "Rainfall vs Malaria"),
              status = "primary", solidHeader = FALSE, width = 6,
              plotlyOutput("plot_rain_malaria", height = "380px"))
        ),
        fluidRow(
          box(title = span(icon("city"), "Monthly Climate by City"),
              ...
              selectInput("city_select", NULL,
                          ...
                          selected = "Nairobi", width = "100%"),
              plotlyOutput("plot_city_climate", height = "320px")),
          box(title = span(icon("fire"), "Climate Anomaly Heatmap"),
      ...
      tabItem(tabName = "regional",
        fluidRow(
          box(title = span(icon("globe-africa"), "Malaria Incidence: Kenya vs East Africa"),
              status = "primary", solidHeader = FALSE, width = 12,
              plotlyOutput("plot_ea_comparison", height = "450px"))
        ),
        fluidRow(
          box(title = span(icon("trophy"), "Country Rankings"),
              status = "info", solidHeader = FALSE, width = 5,
              plotlyOutput("plot_ea_ranking", height = "380px")),
          box(title = span(icon("percentage"), "Trend Comparison (% Change)"),
              status = "warning", solidHeader = FALSE, width = 7,
      ...
      tabItem(tabName = "predictions",
        fluidRow(
          box(title = span(icon("brain"), "Random Forest: Predictions vs Actual"),
              status = "primary", solidHeader = FALSE, width = 8,
              plotlyOutput("plot_predictions", height = "450px")),
          box(title = span(icon("chart-pie"), "Model Performance"),
              ...
              tags$div(style = "padding: 0 10px;",
                tags$table(style = "width: 100%; font-size: 13px;",
                  tags$tr(tags$td(tags$strong("Test R-sq")),   tags$td(style = "text-align: right;", "0.180")),
                  ...
                  tags$tr(tags$td(tags$strong("Features")),    tags$td(style = "text-align: right;", "18")),
        ...
        fluidRow(
          box(title = span(icon("table"), "Prediction Data"),
      ...
      tabItem(tabName = "simulator",
        fluidRow(
          box(title = span(icon("sliders-h"), "Adjust Parameters"),
              status = "danger", solidHeader = FALSE, width = 4,
              tags$div(style = "padding: 5px 0;",
                sliderInput("sim_lag1", "Last Year's Incidence (per 1,000):", value = 74, min = 20, max = 250, step = 1, post = " /1K", width = "100%"),
                sliderInput("sim_precip", "Annual Rainfall (mm):", value = 1500, min = 500, max = 3000, step = 50, post = " mm", width = "100%"),
                sliderInput("sim_temp", "Mean Temperature (C):", value = 21.3, min = 15, max = 30, step = 0.1, post = "C", width = "100%"),
                sliderInput("sim_humidity", "Relative Humidity (%):", value = 73, min = 40, max = 100, step = 1, post = "%", width = "100%"),
                sliderInput("sim_population", "Population (millions):", value = 57, min = 30, max = 70, step = 0.5, post = "M", width = "100%"),
                sliderInput("sim_urban", "Urbanization (%):", value = 32.5, min = 15, max = 50, step = 0.5, post = "%", width = "100%"),
                tags$hr(style = "border-color: #eee;"),
                actionButton("run_sim", "Predict Risk", icon = icon("play"), width = "100%", class = "btn-danger"))),
          box(title = span(icon("chart-line"), "Simulation Result"),
              status = "primary", solidHeader = FALSE, width = 8,
      ...
      tabItem(tabName = "about",
        fluidRow(
          box(title = span(icon("info-circle"), "About This Project"),
              status = "info", solidHeader = FALSE, width = 12,
              tags$div(style = "max-width: 800px; line-height: 1.8;",
                tags$h3("Malaria Outbreak Predictor", style = "font-weight: 700;"),
                ...
                tags$p("Malaria kills over 600,000 people annually, with 90% of deaths in sub-Saharan Africa. This system predicts outbreak risk from climate and health data."),
                ...
                tags$div(style = "display: flex; gap: 10px; flex-wrap: wrap; margin-top: 10px;",
                  ...
                  tags$span(style = "background: #f3e5f5; color: #7b1fa2; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 500;", "tidymodels"),
                ...
                tags$hr(),
                tags$p(icon("code"), " Built by ", tags$strong("Calvin Omondi Okoth"),
                       " | ", tags$a(href = "https://github.com/calvinokoth9528-cloud/malaria-outbreak-predictor", target = "_blank", icon("github"), " GitHub"),
...
server <- function(input, output, session) {
  ...
  output$vb_cases <- renderValueBox({
    ...
    valueBox(format(round(latest / 1e6, 1), nsmall = 1) |> paste0("M"),
             "Annual Cases (2024)", icon = icon("virus"), color = "red")
  })

  output$vb_deaths <- renderValueBox({
    latest <- tail(merged$estimated_malaria_deaths_value, 1)
    valueBox(format(round(latest / 1000, 1), nsmall = 1) |> paste0("K"),
             "Annual Deaths (2024)", icon = icon("skull-crossbones"), color = "black")
  })

  output$vb_incidence <- renderValueBox({
    latest <- tail(merged$malaria_incidence_per_1000, 1)
    valueBox(round(latest, 1) |> as.character(),
             "Incidence per 1,000 (2024)", icon = icon("chart-line"), color = "yellow")
  })
  ...
  output$vb_trend <- renderValueBox({
    ...
    valueBox(paste0(pct, "%"), "Change Since 2000", icon = icon("arrow-down"), color = "green")
  })
  ...
  output$plot_incidence_trend <- renderPlotly({
    p <- ggplot(merged, aes(x = year, y = malaria_incidence_per_1000,
                             text = paste0("Year: ", year, "<br>Incidence: ", round(malaria_incidence_per_1000, 1), " per 1,000"))) +
    ...
    ggplotly(p, tooltip = "text") %>%
      ...
      config(displayModeBar = TRUE, modeBarButtonsToRemove = c("lasso2d", "select2d"), displaylogo = FALSE, responsive = TRUE)
  })

  output$plot_cases_deaths <- renderPlotly({
    p <- merged %>%
      filter(!is.na(estimated_malaria_cases_value), !is.na(estimated_malaria_deaths_value)) %>%
      mutate(cases_M = estimated_malaria_cases_value / 1e6,
      ...
      labs(x = NULL) + theme_minimal(base_size = 13) +
      theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(),
            legend.position = "top", plot.margin = margin(10, 15, 10, 15))
    ggplotly(p, tooltip = "text") %>%
      layout(hoverlabel = list(bgcolor = "white"), margin = list(t = 40, b = 40, l = 60, r = 60)) %>%
  ...
  output$plot_temp_malaria <- renderPlotly({
    p <- merged %>% filter(!is.na(temp_mean_c), !is.na(malaria_incidence_per_1000)) %>%
      mutate(text = paste0("Year: ", year, "<br>Temp: ", round(temp_mean_c, 1), "C<br>Malaria: ", round(malaria_incidence_per_1000, 1))) %>%
    ...
    ggplotly(p, tooltip = "text") %>% config(displayModeBar = FALSE, responsive = TRUE)
  })

  output$plot_rain_malaria <- renderPlotly({
    p <- merged %>% filter(!is.na(precip_total_mm), !is.na(malaria_incidence_per_1000)) %>%
      mutate(text = paste0("Year: ", year, "<br>Rainfall: ", round(precip_total_mm), " mm<br>Malaria: ", round(malaria_incidence_per_1000, 1))) %>%
  ...
  output$plot_city_climate <- renderPlotly({
    ...
    city_data <- climate_m %>% filter(city == tolower(input$city_select)) %>%
      group_by(month) %>%
      summarise(temp = mean(temp_mean_c, na.rm = TRUE), precip = mean(precip_total_mm, na.rm = TRUE),
                humidity = mean(humidity_mean, na.rm = TRUE), .groups = "drop") %>%
      ...
      mutate(label = recode(var, temp = "Temperature (C)", precip = "Rainfall (mm)", humidity = "Humidity (%)"),
             text = paste0(month_name, "<br>", label, ": ", round(val, 1)))
    p <- ggplot(city_data, aes(x = month_name, y = val, color = label, group = label, text = text)) +
      geom_line(linewidth = 1.3) + geom_point(size = 3, fill = "white", stroke = 1.5) +
  ...
  output$plot_anomaly_heatmap <- renderPlotly({
    p <- ml_features %>% filter(!is.na(precip_anomaly), !is.na(temp_anomaly)) %>%
      select(year, precip_anomaly, temp_anomaly) %>%
      pivot_longer(cols = c(precip_anomaly, temp_anomaly), names_to = "var", values_to = "anomaly") %>%
      mutate(label = recode(var, precip_anomaly = "Rainfall", temp_anomaly = "Temperature"),
             text = paste0("Year: ", year, "<br>", label, " anomaly: ", round(anomaly, 1))) %>%
      ggplot(aes(x = factor(year), y = label, fill = anomaly, text = text)) +
      geom_tile(color = "white", linewidth = 0.5) +
      scale_fill_gradient2(low = "#1565c0", mid = "white", high = "#e63946", midpoint = 0, name = "Anomaly") +
      labs(x = NULL, y = NULL) +
  ...
  output$plot_ea_comparison <- renderPlotly({
    p <- ea_data %>% filter(!is.na(malaria_incidence_per_1000)) %>%
      mutate(is_kenya = country == "Kenya",
             alpha_val = ifelse(is_kenya, 1, 0.4),
             text = paste0(country, "<br>Year: ", year, "<br>Incidence: ", round(malaria_incidence_per_1000, 1))) %>%
      ggplot(aes(x = year, y = malaria_incidence_per_1000, color = is_kenya, group = country, alpha = alpha_val, text = text)) +
      geom_line(linewidth = 1) + geom_point(size = 2) +
      scale_color_manual(values = c("TRUE" = "#e63946", "FALSE" = "#bdbdbd"), labels = c("East Africa", "Kenya"), name = NULL) +
      scale_alpha_identity() +
      labs(x = NULL, y = "Incidence per 1,000") +
      theme_minimal(base_size = 13) + theme(legend.position = "top", panel.grid.minor = element_blank())
    ggplotly(p, tooltip = "text") %>% config(displayModeBar = FALSE, responsive = TRUE)
  })

  output$plot_ea_ranking <- renderPlotly({
    latest <- ea_data %>% filter(year == max(year, na.rm = TRUE), !is.na(malaria_incidence_per_1000)) %>%
      arrange(desc(malaria_incidence_per_1000))
    p <- ggplot(latest, aes(x = reorder(country, malaria_incidence_per_1000), y = malaria_incidence_per_1000,
                             fill = country == "Kenya", text = paste0(country, "<br>", round(malaria_incidence_per_1000, 1), " per 1,000"))) +
      geom_col() + coord_flip() +
      scale_fill_manual(values = c("TRUE" = "#e63946", "FALSE" = "#bdbdbd"), guide = "none") +
      labs(x = NULL, y = "Incidence per 1,000") +
      theme_minimal(base_size = 12) + theme(panel.grid.major.y = element_blank(), panel.grid.minor = element_blank())
    ggplotly(p, tooltip = "text") %>% config(displayModeBar = FALSE, responsive = TRUE)
  })

  output$plot_ea_trends <- renderPlotly({
    p <- ea_data %>% filter(!is.na(malaria_incidence_per_1000)) %>% group_by(country) %>%
      mutate(change = ((malaria_incidence_per_1000 - first(malaria_incidence_per_1000)) / first(malaria_incidence_per_1000)) * 100,
             is_kenya = country == "Kenya",
             text = paste0(country, "<br>Year: ", year, "<br>Change: ", round(change, 1), "%")) %>%
      ungroup() %>%
  ...
  output$plot_predictions <- renderPlotly({
    ...
    p <- ggplot() +
      geom_ribbon(data = test, aes(x = year, ymin = pmin(malaria_incidence_per_1000, predicted),
                                   ymax = pmax(malaria_incidence_per_1000, predicted)), fill = "#c8e6c9", alpha = 0.5) +
      geom_line(data = train, aes(x = year, y = malaria_incidence_per_1000,
                                   text = paste0("Year: ", year, "<br>Actual: ", round(malaria_incidence_per_1000, 1))),
                color = "#e63946", linewidth = 1.5) +
      geom_point(data = train, aes(x = year, y = malaria_incidence_per_1000), color = "#e63946", size = 4, fill = "white", stroke = 1.5) +
      geom_line(data = train, aes(x = year, y = predicted,
                                   text = paste0("Year: ", year, "<br>Predicted: ", round(predicted, 1))),
      ...
      theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank())
    ggplotly(p, tooltip = "text") %>%
      layout(hoverlabel = list(bgcolor = "white"), margin = list(t = 10, b = 40, l = 60, r = 20)) %>%
      config(displayModeBar = FALSE, responsive = TRUE)
  })
  ...
  output$dt_predictions <- renderDT({
    predictions %>%
      ...
      datatable(options = list(pageLength = 10, dom = "ftip",
                               columnDefs = list(list(className = "dt-center", targets = 0:4))),
                rownames = FALSE) %>%
      formatStyle("residual",
                  backgroundColor = styleInterval(c(-5, 5),
  ...
  sim_result <- eventReactive(input$run_sim, {
    ...
    pred <- lag1 * 0.45 + (precip / 100) * 2.5 + (temp - 21) * 5 +
      (humidity - 70) * 0.3 + (urban / 10) * (-0.5) + rnorm(1, 0, 1.5)
    pred <- max(10, min(300, pred))

    list(prediction = round(pred, 1),
         risk = case_when(pred < 50 ~ "LOW", pred < 80 ~ "MODERATE", pred < 120 ~ "HIGH", TRUE ~ "VERY HIGH"),
         color = case_when(pred < 50 ~ "#2e7d32", pred < 80 ~ "#f57c00", pred < 120 ~ "#e65100", TRUE ~ "#c62828"),
         bg = case_when(pred < 50 ~ "#e8f5e9", pred < 80 ~ "#fff3e0", pred < 120 ~ "#fbe9e7", TRUE ~ "#ffebee"),
         explanation = case_when(
           pred < 50  ~ "Low risk. Maintain surveillance.",
           pred < 80  ~ "Moderate risk. Increase monitoring. Pre-position diagnostic kits.",
           pred < 120 ~ "High risk. Deploy rapid response teams. Increase bed net distribution.",
           TRUE       ~ "Critical risk. Emergency response needed. Activate all malaria control resources."))
  })

  output$plot_sim_result <- renderPlotly({
    res <- sim_result()
    plot_data <- data.frame(
      year = c(merged$year, 2025),
      incidence = c(merged$malaria_incidence_per_1000, res$prediction),
      type = c(rep("Historical", nrow(merged)), "Simulated"),
      text = c(paste0("Year: ", merged$year, "<br>Actual: ", round(merged$malaria_incidence_per_1000, 1)),
               paste0("Year: 2025<br>Predicted: ", res$prediction)))
    p <- ggplot(plot_data, aes(x = year, y = incidence, color = type, text = text)) +
      geom_line(aes(linewidth = type)) + geom_point(aes(size = type)) +
  ...
  output$sim_risk_box <- renderUI({
    res <- sim_result()
    tags$div(style = paste0("padding: 20px; margin-top: 15px; border-radius: 12px; background: ", res$bg, "; border-left: 5px solid ", res$color, ";"),
      tags$div(style = "display: flex; justify-content: space-between; align-items: center;",
        tags$div(
          tags$div(style = paste0("font-size: 36px; font-weight: 700; color: ", res$color, ";"), paste(res$prediction, "per 1,000")),
          tags$div(style = paste0("font-size: 16px; font-weight: 600; color: ", res$color, ";"), paste("Risk Level:", res$risk)))))
  })

  output$sim_explanation <- renderUI({
    res <- sim_result()
    tags$div(style = paste0("padding: 12px 15px; border-radius: 8px; background: #fafafa; border-left: 3px solid ", res$color, "; font-size: 13px; color: #555;"),
      icon("lightbulb", style = paste0("color: ", res$color)),
